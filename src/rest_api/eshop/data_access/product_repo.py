...
def save(product: Product):
    for i in range(len(_products)):
        ...
        if existed_product.id == product.id:
            # Пересохранить продукт
            _products[i] = product
            break
    else:
        # Добавить продукт. Выполняется только если не сработал break в цикле
        _products.append(product)


def delete_by_id(id: str):
    global _products
    # Формируется новый список из продктов, кроме продукта с указанным id
