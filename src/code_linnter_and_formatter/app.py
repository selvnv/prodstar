...
def create_app():
    ...
    def sglistsingle():
            ...
            if own == "Y":
                ...
                if lowest_comp_price != "NO COMP":
                    ...
                    # positive difference == lower bprice
                        ...
                        # SO THE TEAM WHEN THEY SEE BFLOOR WILL COMPARE TO THE 2ND LOWEST
                        # AND -1 TO THEIR APROX BPRICE THEREFORE AVOIDING ANY WEIRD BPRICE FROM
                        # OTHER BROKERS WHO MAY TRY TO PUSH DOWN OUR PRICING.

                        return fv, "BFLOOR"
                    elif (difference > 0) & (size > ownsize):
                        return round(suggestedbprice, 2), "MRKDWN"
                    # negative difference = increase bprice but only
                    # if difference of more than 5 dollars (to monitor)
                    # need to find the 2nd lowest comp price that is "N" then push that.
                    # this would need to be seperated out as a seperate column
                    # with a different logic behind it.
        ...
        response = requests.get(
            "https://api.blabla.com/event_listings.php?token=blabla&eventid=%s" % (skyid)
        )
        today = datetime.today().strftime("%m%d%Y")
        ...
        if ownsizedf.empty:
            ...
            # to find the Mrkup 2nd lowest price only
            # if there is more than 1 count and if it is nochanges
    ...
    def sglistfull():
        ...
        data_on_scrape_date = pd.DataFrame(fulldata)
        data_on_scrape_date["scrapedate"] = sdate
        print(data_on_scrape_date)
        data_on_scrape_date.rename(columns={0: "SG_EID", 1: "Data"}, inplace=True)
        dataarr = data_on_scrape_date.to_json(index=False, orient="table")
        client.close()
        return dataarr
    ...
    def sgsoldinv():
        ...
        if token == tokenkey:
            ...
            sold_inventory_df = pd.DataFrame(data)
            datajson = sold_inventory_df.to_dict(orient="records")
            client.close()
            return datajson
        else:
    ...
    def sgsoldinvsingle(inid: int):
        ...
        if token == tokenkey:
            ...
            data = soldmongoresults["data"]
            sold_inventory_df = pd.DataFrame(data)
            sold_inventory_df["filled_date"] = pd.to_datetime(
                sold_inventory_df["filled_date"], errors="coerce"
            ).dt.strftime("%m/%d/%Y")
            sold_inventory_df["PURCHASE_DATE"] = pd.to_datetime(
                sold_inventory_df["PURCHASE_DATE"], errors="coerce"
            ).dt.strftime("%m/%d/%Y")
            sold_inventory_df["filled_date"].fillna("NA", inplace=True)
            sold_inventory_df["PURCHASE_DATE"].fillna("NA", inplace=True)
            print(sold_inventory_df["PURCHASE_DATE"])
            print(sold_inventory_df["filled_date"])
            print(sold_inventory_df["PURCHASE_DATE"].dtype)
            print(sold_inventory_df["filled_date"].dtype)
            print(sold_inventory_df.columns)
            dfrow = pd.DataFrame(
                sold_inventory_df[sold_inventory_df["invoiceId"] == str(inid)]
            ).reset_index()
