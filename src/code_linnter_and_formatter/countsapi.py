        ...
        if ed_count > 1:
            for data in results:
                ...
                datadf.drop_duplicates(
                    subset=["CODE", "COUNT", "CAPACITY", "PRICE"], keep="first", inplace=True
                )
                ...
                if broaddf.empty:
                    ...
                    broaddf.drop(
                        broaddf.columns.difference(broaddfcolstokeep), inplace=True, axis=1
                    )
                    ...
                    for f in feesarr:
                        ...
                        if teamname in t:
                            ...
                            fees = 10
                    cleandf["fees"] = fees
                    groubbydf = pd.crosstab(cleandf["CODE"], cleandf["BROADCAST"]).reset_index(
                        names="CODE"
                    )
                    print(groubbydf)
            ...
            for data in results:
                ...
                if skyvssgresults is None:
                    ...
                    if broaddf.empty:
                        ...
                        cleandf.drop(
                            cleandf.columns.difference(finalcolstokeep), axis=1, inplace=True
                        )
                        cleandf = cleandf.reindex(finalcolstokeep, axis=1)
                        cleandf.drop_duplicates(keep="first", inplace=True)
                        cleandf.fillna("No Data", inplace=True)
